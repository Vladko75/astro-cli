...
func main() {
	...
	// Show time
	if showOptions["time"] {
		showCurrentTime(os.Stdout, cityName, t)
	}

	// Show moon
	if showOptions["moon"] {
		showMoonInfo(os.Stdout, cityName, lat, lon, t)
	}

	// Show planets
	if showOptions["planets"] {
		showPlanetsInfo(os.Stdout, lat, lon, t)
	}
}

func showCurrentTime(w io.Writer, cityName string, t time.Time) {
...
func showMoonInfo(w io.Writer, cityName string, lat, lon float64, t time.Time) {
...
func showPlanetsInfo(w io.Writer, lat, lon float64, t time.Time) {
